...
namespace HFIT.Services
{
      ...
      public   class WorkoutService : Service
    {
        ...
        public async Task<Message<List<TreinoModel>>> GetWorkoutById(int CorporacaoId, int Id,int IdTreino, string myToken)
        {
            ...
            try
            {
                ...
                //   response = await _client.GetAsync($"{BaseApiurl}api/Treino/GetAllTreino");
                response = await _client.GetAsync($"{BaseApiurl}api/Treino/GetActiveWorkoutById?Master=1&Corporacao={CorporacaoId}&Treino={IdTreino}&Usercodigo={Id}");
